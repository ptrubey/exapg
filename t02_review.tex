\st{Our analysis of the SLOSH data's extremal dependence requires some background.  
...
\bruno{We consider observations $\bm{w}_{i} = (w_{i1}, \ldots , w_{iS})$, where $i =1 ,\ldots m ,n$. 
For the case of the SLOSH simulations, $i$ indexes storms, and $s$ indexes the grid
cell in the simulation output.}
\st{Let} \bruno{Define as} the threshold $b_{qs} = \hat{F}_{s}^{-1}(1 - q)$, where $\hat{F}$ is
    ...
    Pareto form via the transformation} \bruno{Let}
    \begin{equation}
    ...
    angular component $\bm{V}\in\mathbb{S}_{\infty}^{d-1}$.  } \bruno{The core 
...
A suitable distribution for $\bm{V}$ can be approximated by projecting a 
    ...
    \st{succeed in establishing} \bruno{obtain} a distribution for $\bm{y}$ on 
    $\mathbb{S}_{p}^{d-1}$.
    ...
    form, \st{leaving} \bruno{yielding} the \emph{projected gamma} density for arbitrary $p > 0$,
    \[
    ...
    \]
    \st{Of course,} \bruno{Unfortunately,} at $p=\infty$, the transformation is not differentiable, so a 
    direct projection onto $\mathbb{S}_{\infty}^{d-1}$ is not possible. \st{We
    instead desire a high but finite $p$.
    Here we balance two issues:} \bruno{Yet,} as $p\to\infty$, \st{the space} $\mathbb{S}_{p}^{d-1}$ 
...
To more \st{aithfully} \bruno{flexibly} capture the structure of the data, we use the projected gamma density 
    ...
    denotes the \emph{concentration} parameter. \bruno{Notice that $J$ dennotes a
    truncation for the number of mixture components and, thus, $j\in \lbrace 1,\ldots,J\rbrace$ 
    denotes indexing over \emph{clusters}.}

\st{For the rest of this paper, let $i$ denote indexing over storm simulations (observations), 
    ...
    over locations t which the storm surge is simulated.  That is,} \bruno{Let} $y_{is}$ \st{is the maximum 
    simulated} \bruno{be the }
    storm surge at location $s$ during storm $i$, after \bruno{thresholding  and }projection onto $\mathbb{S}_p^{d-1}$.
...
Such a fitting scheme can work well for a moderately sized inference problem.  In \cite{trubey:pg},
    ...
    memory resources to hold the sampling history} \bruno{will have an increasing memory footprint. 
    Thus, we consider, as an alternative, a variational inference approach.}
    \st{We consider an alternative method of model fitting.}

\subsection{Variational Inference - A Brief Overview\label{ref:varbayes}}
Variational inference, or variational Bayesian statistics, is an alternative method of 
    ...
    as possible. \bruno{\bf You need some references here.} That is, for data $\bm{x}$ and some distributional parameter set $\bm{\theta}$,
...
\subsection{Sea, Lake, and Overland Surge from Hurricanes (SLOSH)\label{ref:slosh}}
