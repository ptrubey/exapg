...
Variational inference, or variational Bayesian statistics, is an alternative method of 
    ...
    as possible. \bruno{\bf You need some references here.} That is, for data $\bm{x}$ and some 
    distributional parameter set $\bm{\theta}$, where $f(\bm{\theta}\mid \bm{x})$ is not 
    available in closed form, then we select
    \begin{equation}
        \label{eqn:optimalq}
...
\subsection{\bruno{Extreme value analysis of SLOSH output}}
...
Figure~\ref{fig:thetahistogram} shows the marginal histograms of \st{storm} \bruno{SLOSH input} parameters, for 
    storms which exhibited extreme behavior in the \emph{Threshold} slice.  Storm parameters 
    in the \st{originating data} \bruno{original simulation} were sampled via Latin hypercube, so would appear marginally
