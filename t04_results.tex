...
The range of latitudes at which storms make landfall in the simulation cover a region of
    ...
    with a classification of those sites.  \bruno{Four categories were considered: ``Services'', that}
    includes emergency services, police, fire, and medical services\bruno{;  ``Land'', that}
    includes village centers, major intersections, and named land features\bruno{; ``Transportation, 
    that}  includes ferry landings, airports, heliports, and similar.
    Within \st{that set, we further} \bruno{transportation category we} identify two 
